(ns bookpubops.governor-contract-test
  ...
  (testing "clean production-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-production-record :title-id "title-1" :patch {:manuscript-status "final draft" :print-run 5000}}
                               ctx)]
      (is (some? result))
  ...
  (testing "clean production-operation scheduling proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1b"
                               {:op :schedule-production-operation :title-id "title-1" :patch {:stage "typesetting" :date "2026-08-01"}}
                               ctx)]
      (is (some? result))
      (is (> (count (store/coordination-log db)) 0)
          "commit must append record to coordination-log"))))
  ...
  (testing "clean distribution-coordination proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1c"
                               {:op :coordinate-distribution :title-id "title-1" :patch {:channel "ebook" :release-date "2026-09-15"}}
                               ctx)]
      (is (some? result))
      (is (> (count (store/coordination-log db)) 0)
          "commit must append record to coordination-log"))))
  ...
  (testing "unregistered title -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t3"
                     {:op :log-production-record :title-id "unknown-title"
                      :patch {:manuscript-status "draft"}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-production-record :title-id "title-1"
                                :patch {:manuscript-status "final draft"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into editorial-content/legal-clearance/rights-grant scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-production-record :title-id "title-1" :patch {:manuscript-status "final draft"}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-production-record :title-id "unknown" :patch {:manuscript-status "draft"}}
                     ctx)
