(ns bookpubops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (BookPubAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BookPubGovernor (:govern)
  ...
  human operator/publishing coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [bookpubops.advisor :as advisor]
            [bookpubops.governor :as governor]
            [bookpubops.phase :as phase]
...
(defn build
  ...
  `bookpubops.store/Store`).
  opts:
    :advisor      -- a `bookpubops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BookPubAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BookPubGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :title-id (:title-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :title-id (:title-id request) :by (:by approval)}]}
            {:disposition :hold
